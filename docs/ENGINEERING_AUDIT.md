...
| UI-5 | `cache.match(url.pathname, { ignoreSearch: true })` can return a cached RSC flight payload (`/today?_rsc=…`) for a document navigation; Next 16 disambiguates RSC by query param and no longer sets `Vary: RSC`. | P1 | sw.js:121 |
