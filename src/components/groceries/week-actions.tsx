...
import { CheckCircle2, MoreVertical, PackagePlus } from 'lucide-react';
import {
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent title="List options">
          <div className="space-y-2">
            {status !== 'COMPLETED' ? (
              <Button variant="outline" size="block" disabled={setStatus.isPending} onClick={() => setStatus.run({ id: weekId, status: 'COMPLETED' })}>
                <CheckCircle2 className="size-4" />
                Mark shopping as done
              </Button>
            ) : (
              <Button variant="outline" size="block" disabled={setStatus.isPending} onClick={() => setStatus.run({ id: weekId, status: 'ACTIVE' })}>
                Reopen this list
              </Button>
            )}

            <Button variant="outline" size="block" disabled={stock.isPending} onClick={() => stock.run({ id: weekId })}>
            ...
            <DeleteButton
              ...
              description="The list and its ticks are removed. Your plan, inventory and past days are unchanged. This cannot be undone."
              pending={remove.isPending}
              onConfirm={() => remove.run({ id: weekId })}
              size="block"
            />
          </div>
        </SheetContent>
