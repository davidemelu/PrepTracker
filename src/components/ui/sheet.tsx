      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-border p-4">
          ...
          <DialogPrimitive.Close
            className="-m-2 flex size-10 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            aria-label="Close"
          >
            <X className="size-5" />
          </DialogPrimitive.Close>
...
export function SheetFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'sticky bottom-0 -mx-4 -mb-4 mt-4 flex gap-2 border-t border-border bg-card p-4 safe-bottom',
        className,
      )}
      {...props}
    />
